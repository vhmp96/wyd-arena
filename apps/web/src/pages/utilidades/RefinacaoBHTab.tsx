...
const UPGRADE_LEVELS = Object.keys(BH_BASE_RATES);
...
export function RefinacaoBHTab() {
  const [upgrade, setUpgrade] = useState('+1 → +5');
  const [soul, setSoul] = useState(0);
  const [simAttempts, setSimAttempts] = useState(10);

  const base = BH_BASE_RATES[upgrade];
  const bonusPerLevel = SOUL_BONUS_PER_LEVEL[upgrade];
  const soulBonus = soul * bonusPerLevel;
    ...
    <div className="space-y-6">
      <div className="space-y-1">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-semibold text-base">Refinação — Bahamut</h2>
        </div>
        <p className="text-xs text-muted-foreground">
          Selecione o nível de upgrade e o nível do Bahamut Soul para ver a chance de sucesso.
        </p>
      </div>
      ...
      <div className="rounded-lg border border-border bg-card p-4 space-y-3">
        <div className="flex items-center justify-between">
          <p className="text-sm font-medium">Bahamut Soul</p>
          <span className="text-xs text-muted-foreground">
            +{bonusPerLevel}% por nível
          </span>
        </div>
      ...
      <div className="rounded-lg border border-border bg-card p-4 space-y-3">
        ...
        <div className="space-y-2 text-sm">
          ...
          <div className="flex justify-between">
            <span className="text-muted-foreground">Bônus Bahamut Soul (+{soul})</span>
            <span className={soulBonus > 0 ? 'text-green-400' : 'text-muted-foreground'}>
              +{soulBonus % 1 === 0 ? soulBonus : soulBonus.toFixed(2)}%
            </span>
          </div>
          <div className="flex justify-between items-center border-t border-border pt-2">
            ...
            <span className={`text-2xl font-bold ${getChanceColor(chanceFinal)}`}>
              {chanceFinal % 1 === 0 ? chanceFinal : chanceFinal.toFixed(2)}%
            </span>
          </div>
        </div>
